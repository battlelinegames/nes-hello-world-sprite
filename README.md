# NES Hello World with Sprites
A Hello World App written in 6502 assembly for the Nintendo Entertainment System and NES Emulators.  Unlike my previous Hello World
NES App, this one is designed to use sprites and not background tiles.  Because the NES only supports 8 sprites per scanline, any more
than 8 characters printed will result in flickering sprites on the left and right side.  If there are more than 16 characters on a
scanline, some characters in the middle will not be shown at all.  I'm using a very simple OAM Cycling strategy that involves mirroring 
the OAM memory on each frame.
...
In this file I create the macro print_sprites.  I use this to place the sprites like a string.  
Because it uses sprites, any more than 8 sprites on a line causes the sprites on the outside
to flicker.  If there are more than 16 sprites on a line any additional sprites will not be shown.
